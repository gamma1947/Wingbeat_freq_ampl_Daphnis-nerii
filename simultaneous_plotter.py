...
class VideoGraphSyncApp:
    def __init__(self, root, video_path, csv_path, output_video_path, output_frame_rate):
        self.root = root
    ...
    def start_video(self):
        if not self.is_playing:
            ...
            if self.video_writer is None:
                ...
                video_width = 640
                graph_width, graph_height = self.canvas.get_width_height()
                total_width = video_width + graph_width
                total_height = 480  # Height of the video
                self.video_writer = cv2.VideoWriter(self.output_video_path, fourcc, self.output_frame_rate, (total_width, total_height))
            self.update_frame()
    ...
    def update_frame(self):
        if not self.is_paused and self.start_frame <= self.frame_number <= self.end_frame:
            ...
            if ret:
                ...
                # Capture the frame and add to video writer
    ...
    def capture_frame(self, frame):
        ...
        # Convert combined PIL image to OpenCV format
        combined_cv_image = np.array(combined_image)
        combined_cv_image = cv2.cvtColor(combined_cv_image, cv2.COLOR_RGB2BGR)

        # Write the combined frame to video
        self.video_writer.write(combined_cv_image) # type: ignore
...
if __name__ == "__main__":
    ...
    output_video_path = r'C:\analysis\app\assets\video_graph_sync_3.avi'  # Path to save the output video
    output_frame_rate = 1000  # Frame rate for the output video (reduce for slower playback)
    app = VideoGraphSyncApp(root, video_file_path, csv_file_path, output_video_path, output_frame_rate)
    root.mainloop()
